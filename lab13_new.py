...
class Maze:
    ...
    def bitmap(self):
        n = self.n
        H = 2 * n + 1
        B = [[1] * H for _ in range(H)]
        for r in range(n):
            for c in range(n):
                br, bc = 2 * r + 1, 2 * c + 1
                B[br][bc] = 0
                if 'N' not in self.w[r][c]:
                    B[br - 1][bc] = 0
                if 'S' not in self.w[r][c]:
                    B[br + 1][bc] = 0
                if 'W' not in self.w[r][c]:
                    B[br][bc - 1] = 0
                if 'E' not in self.w[r][c]:
                    B[br][bc + 1] = 0
        return B
...
class App:
    ...
    def gen(self):
        ...
        for _ in range(100):
            ...
            g = self.rng.choice(odd)
            self.start = (0, s)
            self.goal = (N - 1, g)
            self.m.w[0][s].discard('N')
            self.m.w[N - 1][g].discard('S')
        ...
        self.draw()
        self.st.set(f'Prim+Braid лабиринт {N}×{N}. Вход ↑{s}, выход ↓{g}.')

    def _has_path(self, start, goal):
        # простая проверка наличия пути (итеративная, без анимации)
        stack = [start]
        seen = {start}
        while stack:
            r, c = stack.pop()
    ...
    def start_search_recursive(self):
        ...
        if found:
            ...
            self.path = path[::-1]
            self.st.set('Таракан нашёл выход!')
        else:
...
def main():
    root = tk.Tk()
    root.title('Робот — лабиринт 10×10 (рекурсивный DFS)')
